...
const VendorOrders = () => {
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    try {
      ...
      alert("Update failed: Make sure the status value is accepted by the server.");
    } finally {
      setIsUpdating(false);
    }
  };

  if (isLoading) return <div className="flex justify-center p-20 text-emerald-600"><Loader2 className="animate-spin" size={40} /></div>;
  ...
  return (
    <div className="p-4" dir="ltr">
      ...
      {selectedOrder && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex justify-center items-center p-4" dir="ltr">
          <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden">
            <div className="p-6 border-b flex justify-between items-center">
              <h2 className="text-xl font-bold">Update Order #{selectedOrder.orderId}</h2>
              <button onClick={() => setSelectedOrder(null)}><X size={24} /></button>
            </div>
            <div className="p-6 space-y-4">
              <div className="bg-gray-50 p-4 rounded-xl">
                <p className="text-sm font-bold mb-3">Select New Status:</p>
                  ...
                  {['Shipped', 'Delivered', 'Cancelled'].map((status) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg text-sm font-bold border transition-all ${
                        selectedOrder.status === status ? 'bg-emerald-600 text-white' : 'bg-white hover:border-emerald-600'
                      }`}
                    >
                      {status}
                    </button>
